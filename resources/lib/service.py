...
class OnUpdateChanges:
	...
	def refresh_addon_keys(self):
		# Detects legacy keys from Tikipeter's Fen Light or AnonyMouse's Fen Light AM and clears them.
		# Mire Sirens ships no provider keys; users must supply their own Trakt and TMDB credentials.
		...
		show_dialog = False
		legacy_tmdb_keys = (
			'b370b60447737762ca38457bd77579b3',  # Tikipeter
			'b14c2656f72e5bab233def36928a202b',  # AnonyMouse
		)
		legacy_trakt_clients = (
			'1038ef327e86e7f6d39d80d2eb5479bff66dd8394e813c5e0e387af0f84d89fb',  # Tikipeter
			'c787278de2e4bba0a92125433e89e0d71d89a09a2f9c44dd69c2f13c8e8eef06',  # AnonyMouse
		)
		if get_setting('fenlight.tmdb_api') in legacy_tmdb_keys:
			...
			show_dialog = True
		if get_setting('fenlight.trakt.client') in legacy_trakt_clients:
			...
			show_dialog = True
		if show_dialog:
			text = 'Welcome to Mire Sirens, a community-maintained continuation of the Fen Light addon family. ' \
			'For your security, all previous shared API keys (Trakt, TMDb) have been removed.'
			kodi_utils.ok_dialog(heading='Welcome to Mire Sirens', text=text)
			text = 'You will need to supply your own credentials:[CR]' \
			'- TMDb v3 API Key (for metadata)[CR]' \
			'- TMDb v4 Read Access Token (for lists/favorites - prompted on first use)[CR]' \
			'- Trakt Client ID and Secret (for sync)[CR][CR]' \
			'See the README for free registration links.'
			kodi_utils.ok_dialog(heading='Bring Your Own Keys', text=text)
